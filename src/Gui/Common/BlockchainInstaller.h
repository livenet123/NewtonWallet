...
private:
  ...
  const QDir m_intrinsiccoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfIntrinsiccoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _intrinsiccoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo intrinsiccoinBlockchainInfo() const;
